...
def loss2(recon_img, original_img):
  ...
  vgg16_model.eval()
  recon_img_feature = vgg16_model(recon_img).to(device) # (B,512,16,16) If input (B,3,256,256)
  original_img_feature = vgg16_model(original_img).to(device)
...
def loss3(optical_flow1, optical_flow2, deformable_conv, M=10):
  ...
  dx1 = deformable_conv[:,0:9,31,31].to(device) #(B,9)
  dy1 = deformable_conv[:,9:18,31,31].to(device)
  dx2 = deformable_conv[:,0:9,31,95].to(device)
  dy2 = deformable_conv[:,9:18,31,95].to(device)
  dx3 = deformable_conv[:,0:9,95,31].to(device)
  dy3 = deformable_conv[:,9:18,95,31].to(device)
  dx4 = deformable_conv[:,0:9,95,95].to(device)
  dy4 = deformable_conv[:,9:18,95,95].to(device)
  ...
  for i in range(2):
    ...
    # optical flow u,v
    op1_u = optical_flow[:,0,30:33,30:33].reshape(-1,9).to(device) #(B,9)
    op1_v = optical_flow[:,1,30:33,30:33].reshape(-1,9).to(device)
    op2_u = optical_flow[:,0,30:33,94:97].reshape(-1,9).to(device)
    op2_v = optical_flow[:,1,30:33,94:97].reshape(-1,9).to(device)
    op3_u = optical_flow[:,0,94:97,30:33].reshape(-1,9).to(device)
    op3_v = optical_flow[:,1,94:97,30:33].reshape(-1,9).to(device)
    op4_u = optical_flow[:,0,94:97,94:97].reshape(-1,9).to(device)
    op4_v = optical_flow[:,1,94:97,94:97].reshape(-1,9).to(device)
    ...
    mask = x0.clone().to(device) #(B,4*9) # mask=1 if x0 in [min(0,u),max(0,u)]; else mask=0
